    ...
    @if (auth()->user()->isSupporter())
        ...
        >
            ...
            <div
                ...
                style="display:none;position:absolute;bottom:calc(100% + 6px);left:0;right:0;background:var(--surface);border:1px solid var(--border);border-radius:14px;z-index:50;box-shadow:0 8px 24px rgba(0,0,0,0.5);"
            >
                {{-- Collections list --}}
                        ...
                        @foreach ($this->collections as $collection)
                            ...
                            >
                                <span class="flex-none w-3 h-3 rounded-sm border flex items-center justify-center"
                                    style="border-color:{{ $collection->in_collection ? 'var(--accent)' : 'var(--border)' }};background:{{ $collection->in_collection ? 'var(--accent)' : 'transparent' }};"
                                >
                                    @if ($collection->in_collection)
                                        <svg width="8" height="8" viewBox="0 0 12 12" fill="none" stroke="var(--on-accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 6l3 3 5-5"/></svg>
                                    @endif
                                </span>
                ...
                @if ($showForm)
                    <div class="px-3 py-2 flex gap-2">
                        ...
                        />
                        <x-button type="button" wire:click="create" class="!text-xs !px-2 !py-1">Add</x-button>
                    </div>
                @else
                    <button
                        type="button"
