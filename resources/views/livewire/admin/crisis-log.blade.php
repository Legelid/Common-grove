<div class="p-8">
    <h1 class="text-2xl font-bold mb-4" style="color:#E6EDF3;">Crisis Log</h1>
    ...
    @if ($this->detections->isEmpty())
        <p class="text-sm" style="color:#8B949E;">No crisis keyword detections on record.</p>
    @else
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y" style="divide-color:#30363D;">
                    @foreach ($this->detections as $detection)
                        <tr wire:key="detection-{{ $detection->id }}" style="border-color:#30363D;">
                            <td class="py-3 pr-6 text-xs whitespace-nowrap" style="color:#8B949E;">
                                {{ $detection->detected_at->format('Y-m-d H:i') }}
                            </td>
                            <td class="py-3 pr-6">
                                @if ($detection->user)
                                    <a href="{{ route('profile.show', $detection->user->gamertag) }}" target="_blank"
                                        class="text-sm font-medium transition" style="color:#1D9E75;"
                                        onmouseover="this.style.color='#22B88A'" onmouseout="this.style.color='#1D9E75'"
                                    >{{ $detection->user->gamertag }}</a>
                                @else
                                    <span class="text-sm" style="color:#30363D;">Deleted user</span>
                                @endif
                            </td>
                            <td class="py-3 pr-6 text-xs font-mono" style="color:#E6EDF3;">
                                {{ $detection->triggered_keyword }}
                            </td>
                            <td class="py-3">
                                @if ($detection->was_dismissed)
                                    <span class="px-2 py-0.5 text-xs rounded-full" style="background:#21262D;color:#8B949E;">Dismissed</span>
                                @else
